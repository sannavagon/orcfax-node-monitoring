...
if [ -z "$1" ]; then
     ...
     echo "❌ Incorrect format. Use: ./monitoring_orcfax.sh [YYYY-MM-DD]
[HH]"
     exit 1
fi
...
LOGS=$(journalctl -u orcfax-collector.service --since "$SINCE" --until
"$UNTIL" --no-pager 2>/dev/null)
...
TIMEOUT_ERRORS=$(echo "$LOGS" | grep "ERROR ::" | grep "wait_for resp
timeout" | wc -l)
...
if [ "$TIMEOUT_ERRORS" -gt 0 ]; then
     ...
     echo "$LOGS" | grep "ERROR ::" | grep "wait_for resp timeout" | awk
-F"::" '{print $3}' | sort | uniq -c | sort -nr
     echo ""
else
...
WEBSOCKET_ERRORS=$(echo "$LOGS" | grep "ERROR ::" | grep "websocket
response: OK" | wc -l)
OTHER_ERRORS=$(echo "$LOGS" | grep "ERROR ::" | grep -v "wait_for resp
timeout" | grep -v "websocket response: OK" | wc -l)
...
if [ "$WEBSOCKET_ERRORS" -gt 0 ] || [ "$OTHER_ERRORS" -gt 0 ]; then
     ...
     if [ "$WEBSOCKET_ERRORS" -gt 0 ]; then
         echo "$LOGS" | grep "ERROR ::" | grep "websocket response: OK"
| awk -F"::" '{print $3}' | sort | uniq -c | sort -nr
     fi
     if [ "$OTHER_ERRORS" -gt 0 ]; then
         echo "$LOGS" | grep "ERROR ::" | grep -v "wait_for resp
timeout" | grep -v "websocket response: OK" | awk -F"::" '{print $3}' |
sort | uniq -c | sort -nr
     fi
     echo ""
else
...
for PAIR in "ADA/USD" "ADA/BTC" "BTC/USD"; do
     ...
     journalctl -u orcfax-collector.service --since "$SINCE" --until
"$UNTIL" --no-pager 2>/dev/null |
...
show_cex_responses() {
     ...
     echo "✅ $PAIR:"
     echo "-----------------------------------------"
     echo "$LOGS" | grep "compiled URL for $PAIR" | awk -F' ' '{print
$NF}' | awk -F'/' '{print $3}' | sort | uniq -c | while read RESPONSES
CEX; do
         TOTAL_REQUESTS=$(echo "$LOGS" | grep "compiled URL for $PAIR" |
grep "$CEX" | wc -l)
...
echo "$LOGS" | grep "websocket response: OK (" | awk -F"OK \\(" '{print
$2}' | awk -F"\\)" '{print $1}' | sort | uniq -c

echo ""

# Count OK responses from the validator in the same minute for multiple
pairs
echo "Pairs that overlapped in the same minute:"

journalctl -u orcfax-collector.service --since "$SINCE" --until "$UNTIL"
--no-pager 2>/dev/null |
     grep "websocket response: OK" |
     awk '{print substr($1, 1, 16), $NF}' | # Extract EPOCH-MM-DD HH:MM
and pair
     sort | uniq |
     awk '{pairs[$1] = pairs[$1] ? pairs[$1] "," $2 : $2} END {for
(minute in pairs) {split(pairs[minute], arr, ","); if (length(arr) > 1)
{print minute ": " pairs[minute]}}}' | sort
...
VALIDATIONS=$(journalctl -u orcfax-collector.service --since "$SINCE"
--until "$UNTIL" --no-pager 2>/dev/null | grep "websocket
response\|wait_for resp timeout")
...
for MINUTE in $MINUTES; do
     ...
     for PAIR in "ADA-USD" "ADA-BTC" "BTC-USD"; do
         VALIDATION_PAIR=$(echo "$VALIDATIONS" | grep "$TIMESTAMP" |
grep "$PAIR")

         if [[ -n "$VALIDATION_PAIR" ]]; then
             SUCCESSFUL=$(echo "$VALIDATION_PAIR" | grep "OK (" | wc -l)
             FAILED=$(echo "$VALIDATION_PAIR" | grep "wait_for resp
timeout" | wc -l)

             if [[ "$FAILED" -gt 1 ]]; then
                 echo -n "$(tput setaf 4)🟦$(tput sgr0)" # Blue for more
than one timeout
                 TOTAL_MULTIPLE_TIMEOUT=$((TOTAL_MULTIPLE_TIMEOUT + 1))
                 TOTAL_TIMEOUT=$((TOTAL_TIMEOUT + 2))
             elif [[ "$FAILED" -gt 0 ]]; then
                 echo -n "$(tput setaf 1)🟥$(tput sgr0)" # Red for one
timeout
                 TOTAL_TIMEOUT=$((TOTAL_TIMEOUT + 1))
             elif [[ "$SUCCESSFUL" -gt 1 ]]; then
                 echo -n "$(tput setaf 3)🟧$(tput sgr0)" # Orange for
more than one successful in the same minute
                 TOTAL_MULTIPLE_OK=$((TOTAL_MULTIPLE_OK + 1))
                 TOTAL_OK=$((TOTAL_OK + 2))
             elif [[ "$SUCCESSFUL" -gt 0 ]]; then
                 echo -n "$(tput setaf 2)🟩$(tput sgr0)" # Green for one
successful
                 TOTAL_OK=$((TOTAL_OK + 1))
             else
...
echo "Total messages processed: $((TOTAL_OK + TOTAL_TIMEOUT)) (should
match Total messages sent)"

# Count service restarts
STARTED_COUNT=$(echo "$LOGS" | grep "Started orcfax-collector.service" |
wc -l)
